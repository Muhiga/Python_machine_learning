...
data=pd.read_csv('Social_Network_Ads.csv').drop(['User ID', 'Gender'], axis=1)
print(data.head(10))
def distribution_plots(df,output_dir, hue_column=None):
...
#2. output_dir: where the data will be saved
    ...
    for column in data.columns:
        ...
        if pd.api.types.is_numeric_dtype(df[column]):
            sns.histplot(data[column], kde=True, hue=None)
    ...
    print('Pair plot plotted')
    plt.close()

    
    plt.figure(figsize=(12,8))
    sns.countplot(x=data['Purchased'])
    plt.title('Countplot of the Target Column (Purchased)')
    plt.ylabel('Frequency')
    plt.savefig(os.path.join(output_dir, 'Purchased_countplot.png'))
    plt.close()
